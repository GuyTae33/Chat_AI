...
export function updateQuickFromText(text) {
  ...
  if (/(천장\s*높이는?\s*[?？]|천장\s*높이.*(어떻게|얼마|되세요|되나요|알려|몇\s*(mm|cm|미터)|어느\s*정도|대략|아세요)|천장.*몇\s*(mm|cm|미터)\s*(\?|예요|인가요|되)|층고.*(어떻게|얼마|몇|되|어느\s*정도)|천장은\s*[?？])/.test(text)) {
    setCeilingCards({ inline: true }); return;
  }
  /* 설치 지역 — 카드 (배송비 산정용, 시/도 수준만) */
  if (/(설치\s*지역|어느\s*지역|지역.*어디|배송.*지역|어디.*거주|어디.*사세요|지역.*알려|어느\s*도시|사시는\s*곳|시\/?도|지역은\s*[?？]|지역이?\s*어떻게)/.test(text)) {
    setRegionCards({ inline: true }); return;
  }
  /* 신규 입주 아파트 여부 — 견적 직전 10% 할인 적용 판단용 */
  /* 견적 요약문(합계·만원·배송비 등) 섞인 경우 오발동 차단 */
  if (!/(합계|만원|견적서|배송비|구성으로|예상\s*견적|할인\s*후)/.test(text) &&
      /(신규\s*(입주\s*)?아파트|신축\s*아파트|신규\s*입주|입주\s*예정|새\s*아파트|새로\s*입주|신축\s*입주|새\s*집).*(인가요|이세요|이신가요|맞으세요|맞나요|되세요|되시나요|예요|이에요|거예요|\?|？)/.test(text)) {
  ...
  if ((_shapeHits >= 2 && /[?？]|인지|계세요|일까요|하세요|신가요|어떠세요|생각/.test(text)) ||
      /(드레스룸\s*형태|형태.*어떻게|어떤\s*형태|형태.*선택|어느\s*형태|형태로\s*생각|형태.*인지|형태.*계세요|어떤\s*형태로|형태는\s*[?？]|형태.*알려|형태로\s*하실|형태\s*추천)/.test(text)) {
  ...
  if (/(커튼박스|커튼\s*박스|창문\s*위.*(들어간|박스|공간)|천장.*안쪽.*(들어간|박스))/.test(text)) {
    setCurtainCards({ inline: true }); return;
  }
  /* 0단계 메뉴 — "어떻게 도와드릴까요?" 첫 인사 직후 손님 진입 경로 선택 */
  if (!/(합계|만원|견적서|배송비|예상\s*견적)/.test(text) &&
      /(어떻게\s*도와\s*드릴|뭐부터\s*도와|어떤\s*도움|무엇부터|어떻게\s*시작|편하게\s*골라)/.test(text)) {
    setQuick([
      '💰 가격대 추천받기',
      '🪟 형태별 예시 보기',
      '📸 사진 보내고 추천받기',
  ...
  /* 어림 크기 질문 — "작은 편/꽤 넓어요" 같이 일상어로 묻는 케이스 (치수 트리거보다 우선)
     견적 요약문(합계/만원/배송비 등) 오발동 차단 가드 포함 */
  if (!/(합계|만원|견적서|배송비|예상\s*견적)/.test(text) &&
      /(어느\s*정도.*크기|대략.*크기|크기.*어느\s*정도|크기.*생각).*[?？]|(작은\s*편).*(꽤\s*넓|넓은\s*편)|(꽤\s*넓).*(작은\s*편)/.test(text)) {
    setQuick(['작은 편이에요', '꽤 넓어요', '잘 모르겠어요'], true); return;
  }
  ...
  /* 예산 질문 — 카드 UI */
  if (/(예산.*얼마|예산.*어느|어느\s*정도\s*예산|어느\s*정도.*예산|예산.*생각|예산\s*범위|예산.*범위|범위.*예산|어떤\s*예산|얼마.*생각|얼마.*예산|얼마쯤|얼마 정도|희망 금액|희망금액|얼마.*까지)/.test(text)) {
    setBudgetCards({ inline: true }); return;
  }
  /* 희망 설치일정 — 시기 정도만 칩으로 (정확한 날짜는 묻지 않음) */
  if (/(설치\s*일정|설치일정|희망\s*설치일|희망\s*일정|언제\s*설치|언제쯤\s*설치|언제\s*받|설치는\s*언제|일정이?\s*어떻게|일정\s*있으세요|일정.*[?？])/.test(text)) {
    setQuick(['🗓️ 이번 달 안', '📅 다음 달', '🌱 천천히 — 정해지면 알려드릴게요'], true); return;
  }
  /* 견적 요약 확인문 — "이런 구성으로 정리해드릴까요?" 류 (질문형만 매칭, 긍정형 '정리해드릴게요!'는 제외) */
  if (/(정리해\s*드릴(까|지\s*요|지요)|이대로\s*견적|예상\s*견적\s*정리|이런?\s*구성으로\s*(정리|진행|견적|드릴까)|이대로\s*진행할까)/.test(text)) {
  ...
  if (_cPicked.length >= 2 && !_isQuoteSummary && !_isFrameShelfQ && !_isOptionTopic && /[?？]|어떤|좋으세요|중에|느낌|골라|선택|어울/.test(text)) {
    ...
    if (_shelfHit >= 2 && _frameHit === 0) {
      setQuick(['🤔 색상 추천해주세요', '화이트오크', '솔리드화이트', '메이플', '스톤그레이', '진그레이', '다크월넛', '민트그린'], true); return;
    }
    if (_frameHit >= 2 && _shelfHit === 0) {
      setQuick(['🤔 색상 추천해주세요', '화이트', '블랙', '실버', '샴페인골드'], true); return;
    }
    /* 섞이거나 모호하면 계열 4종 (콘셉트 좁힘) */
    setQuick(['🤔 색상 추천해주세요', '화이트 계열', '그레이 계열', '우드 계열', '블랙 계열'], true); return;
  }
  /* 선반 색상 질문 — 견적서 항목 언급이 아닌 실제 질문만 */
  if (/(선반\s*색상.*어떻게|선반\s*색상.*알려|선반\s*색상.*선택|선반\s*색상.*원하|어떤\s*선반\s*색상|선반\s*색상은|선반\s*톤|선반\s*컬러|선반\s*색깔)/.test(text)) {
    setQuick(['🤔 색상 추천해주세요', '화이트오크', '솔리드화이트', '메이플', '스톤그레이', '진그레이', '다크월넛', '민트그린'], true); return;
  }
  /* 프레임 색상 질문 — 선반+프레임 동시 톤 질문도 여기서 잡음 (프레임 4종 우선 노출) */
  if (/(프레임\s*색상.*어떻게|프레임\s*색상.*알려|프레임\s*색상.*선택|어떤\s*프레임\s*색|프레임\s*색상은|프레임\s*색상.*원하|선반이랑\s*프레임\s*색상|프레임이랑\s*선반\s*색상|선반.*프레임.*색상.*원하|선반.*프레임.*색상.*톤|프레임.*색상.*톤|프레임\s*톤|프레임\s*컬러|프레임\s*색깔|기둥\s*색)/.test(text)) {
    setQuick(['🤔 색상 추천해주세요', '화이트', '블랙', '실버', '샴페인골드'], true); return;
  }
  /* 색상 전반 질문 (선반+프레임 동시 언급 또는 일반 색상 질문) */
  if (/(색상.*골라|색상.*선택|어떤\s*색|원하시는\s*색|색상은|색상\s*어떻게|선반이랑\s*프레임|어떤\s*톤|톤은\s*[?？]|컬러는\s*[?？]|색깔은\s*[?？]|색은\s*[?？]|색감.*어떻게|분위기.*어떻게|어떤\s*분위기)/.test(text)) {
    setQuick(['🤔 색상 추천해주세요', '화이트 계열', '그레이 계열', '우드 계열', '블랙 계열'], true); return;
  }
  ...
  if (/옵션/.test(text) && /[?？]|있으세요|있을까|있나|원하|어떠세요|생각|필요|추가|넣을|넣고/.test(text)
      ...
      || /(옵션.*추가|어떤\s*옵션|옵션.*뭐|옵션.*선택|옵션.*원하|원하시는?\s*옵션|옵션\s*있|옵션이?\s*있으세요|옵션.*궁금|옵션은\?|구성.*원하|뭐\s*넣|추가.*원하시는|추가하고\s*싶|더\s*넣고\s*싶|구성에\s*뭐)/.test(text)) {
    setOptionCards({ inline: true }); return;
  }
  /* 선반 단수 질문 */
  if (/(선반.*몇\s*단|선반.*단수|몇\s*단으로|단수.*어떻게|단수.*선택|몇단|선반.*몇\s*칸|선반.*몇\s*층|단수는\s*[?？]|선반\s*층수)/.test(text)) {
